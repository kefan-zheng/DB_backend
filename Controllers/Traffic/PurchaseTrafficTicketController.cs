...
namespace LvDao.Controllers
{
    ...
    public class PurchaseTrafficTicketController : ControllerBase
    {
        ...
        [HttpGet("{vehicleId_userId}")]
        public async Task<ActionResult<IEnumerable<LD_PURCHASE_TRAFFIC_TICKET>>> GetPurchaseTrafficTicket(string vehicleId_userId)
        {
            SqlSugar c = new();
            var db = c.GetInstance();
            ...
            var res = await db.Queryable<LD_PURCHASE_TRAFFIC_TICKET>().Where(it => it.USER_ID == user_id && it.VEHICLE_ID==vehicle_id).ToListAsync();
            if (res == null)
            {
                return NotFound();
            }
        ...
        public async Task<ActionResult<LD_PURCHASE_TRAFFIC_TICKET>> PostPurchaseTrafficTicket(LD_PURCHASE_TRAFFIC_TICKET purchase_traffic_ticket)
        {
            ...
            {
                if (db.Queryable<LD_PURCHASE_TRAFFIC_TICKET>().Where(it => it.USER_ID == purchase_traffic_ticket.USER_ID && it.VEHICLE_ID==purchase_traffic_ticket.VEHICLE_ID).Any())
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtAction(nameof(GetPurchaseTrafficTicket), new { seat_id = purchase_traffic_ticket.USER_ID,vehicle_id=purchase_traffic_ticket.VEHICLE_ID }, purchase_traffic_ticket);
        }
        ...
        public async Task<IActionResult> PutPurchaseTrafficTicket(string vehicleId_seatId, LD_PURCHASE_TRAFFIC_TICKET purchase_traffic_ticket)
        {
            ...
            if (seat_id != purchase_traffic_ticket.USER_ID || vehicle_id!=purchase_traffic_ticket.VEHICLE_ID)
            {
                return BadRequest();
            }
            SqlSugar c = new();
            ...
            {
                if (!db.Queryable<LD_PURCHASE_TRAFFIC_TICKET>().Where(it => it.USER_ID == seat_id && it.VEHICLE_ID==vehicle_id).Any())
                {
                    return NotFound();
                }
                else
        ...
        public async Task<IActionResult> DeletePurchaseTrafficTicket(string vehicleId_userId)
        {
            ...
            await Task.Run(() => db.Deleteable<LD_PURCHASE_TRAFFIC_TICKET>().Where(it => it.USER_ID == user_id && it.VEHICLE_ID == vehicle_id).ExecuteCommand());
            return NoContent();
        }
    }
}
