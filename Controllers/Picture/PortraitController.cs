...
namespace LvDao.Controllers
{
    ...
    public class PortraitController : ControllerBase
    {
        ...
        public string GetPic(string id)
        {
            ...
            var table = db.Queryable<LD_PORTRAIT>().Where(it => it.USER_ID == id)
                .ToList();
        ...
        //POST: api/
        [HttpPost("{id}")]
        public async Task<ActionResult<LD_PORTRAIT>> PostPic(string id)
        {
            ...
            LD_PORTRAIT pic_test = new LD_PORTRAIT();
        ...
        public async Task<IActionResult> DeleteMoment(string id)
        {
            SqlSugar c = new();
            var db = c.GetInstance();
            var res = await db.Queryable<LD_PORTRAIT>().Where(it => it.USER_ID == id).ToListAsync();
            if (res == null)
            {
                return NotFound();
            }
            await Task.Run(() => db.Deleteable<LD_PORTRAIT>().In(id).ExecuteCommand());
            return NoContent();
        }
    }
}
